...
@section('content') 
    <table class="table table-resposive text-center" style="font-family: 'Times New Roman', Times, serif">
        ...
        <tbody > 
            ...
            <tr>
                ...
                {{-- <th enctype="multipart/form-data" >{{$item->gambar_15480}}</th> --}}
                <th>
                    @if ($item->gambar_15480)
                    <img style='max-height:50px;max-width:50px' src='{{ url('storage/' . $item->gambar_15480) }}'/>
                    @endif
                <th>
                    <a href="/barang/{{$item->id_15480}}/edit" class="btn btn-sm btn-warning">Update</a>
                    <form action="/barang/{{$item->id_15480}}" method="post" class="d-inline-block">
                        @method('delete')
                        @csrf
    ...
    <a href="/barang/create" class="btn text-white" style="background-color: #F58452; text-decoration:none; margin-left:15px;"> Create</a>
@endsection